    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if new_position in self.corners:
                ...
                new_visited_corners[self.corners.index(new_position)] = True
                new_visited_corners = tuple(new_visited_corners)
            else:
                new_visited_corners = visited_corners
            successors.append(((new_position, new_visited_corners), action,  1))
...
def cornersHeuristic(state, problem):
    ...
    distances = list(
        util.manhattanDistance(currentPosition, unvisited)
...
def foodHeuristic(state, problem):
    ...
    if not foodPositions:
        return 0
    distances = (
        util.manhattanDistance(position, food)  for food in foodPositions
    )
    return max(distances)
